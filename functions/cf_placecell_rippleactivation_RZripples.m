function cf_placecell_rippleactivation_RZripples(dirs, params, allindex, metadata)
...
savefilename = 'ripple_activation_coactivation_placecells_RZ.mat';
...
if ~exist([savedatadir, savefilename]) || rewrite.files
    ...
    for d = 1:size(dayindex,1)
        ...
        decdir = ['\\ad.gatech.edu\bme\labs\singer\Abby\code\chronicflicker-ephys-prospectivecoding\results\decoding_ripples\'];
        load([decdir, 'rippledecoding_halfratiozones_180_PC_', num2str(dayindex(d,2)), '.mat']);
        ...
        for f = 1:length(files)
            ...
            %%% get ripple position
            if length(dayData) >= f
                ripplePosition = dayData(f).rippleposition;
            else
                continue
            end
            
            if length(ripplePosition) ~= size(rippleTimes,1)
                disp('lengths not equal')
                A = 1; %just for the debugger
            end
            inclRipplesPos = ripplePosition >= 0 & ripplePosition < 18;
            rippleTimes = rippleTimes(inclRipplesPos,:);
    ...
    statsfilename = 'TableData_ripples_activation_placecells_RZ.txt';
    writetable(ActivityData, fullfile(statsdir, statsfilename));
    
    statsfilename = 'TableData_ripples_coactivation_placecells_RZ.txt';
...
figure
g.draw();
savefigALP(savefigdir, 'activationprobability_RZ')
...
figure
g.draw();
savefigALP(savefigdir, 'activationprobability_peranimal_RZ')
...
figure
g.draw();
savefigALP(savefigdir, 'activationprobability_byregion_RZ')
...
figure
g.draw();
savefigALP(savefigdir, 'activationprobability_byregion_peranimal_RZ')
...
figure
g.draw();
savefigALP(savefigdir, 'coactivationprobability_RZ')
...
figure
g.draw();
savefigALP(savefigdir, 'coactivationprobability_peranimal_RZ')
...
figure
g.draw();
savefigALP(savefigdir, 'coactivationprobability_byregion_RZ')
...
figure
g.draw();
savefigALP(savefigdir, 'coactivationprobability_byregion_peranimal_RZ')
...
figure
g.draw();
savefigALP(savefigdir, 'coactivationprobability_byregion_dayavg_RZ')
...
figure
g.draw();
savefigALP(savefigdir, 'activationprobability_byregion_dayavg_RZ')

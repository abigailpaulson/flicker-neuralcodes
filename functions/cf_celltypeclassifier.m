function out = cf_celltypeclassifier(dirs, indices, params)
%SP 3.14.19
%Updated ALP 1/15/2020 for K2 datastructures
%modified from runcelltypeclassifier_K2
%
%ALP 12/14/22

%% initialize variables
savedatadir = [dirs.savedatadir 'celltypes\' params.brainReg, '\'];
...
%     allrecProps(i).stabletimes = clustermetrics2mat(clusterInfo(i).metrics, 'stable', 'times');
    allrecProps(i).meanFR  = [clusterInfo(i).metrics.firingrate]; %ALP 12/14/2022
    ...
    filename = ['distFR_mean_A', num2str(dayindex(i,1)), '_', num2str(dayindex(i,2))];
    savefigALP([savedatadir, 'FRDistributions\'], filename, 'filetype', 'pdf')
    end
end
...
saveas(gcf,filename,'fig'); saveas(gcf,filename,'pdf');
close all
...
for i = 1:size(dayindex,1)
    ...
    filename = ['peak2troughDiff_dist_A', num2str(dayindex(i,1)), '_', num2str(dayindex(i,2))];
    savefigALP([savedatadir, 'SpikewidthDistributions\'], filename, 'filetype', 'pdf')
    end
end
...
for i = 1:size(dayindex,1)
    ...
    filename = ['distAC_A', num2str(dayindex(i,1)), '_', num2str(dayindex(i,2))];
    savefigALP([savedatadir, 'AutocorrelogramDistributions\'], filename, 'filetype', 'pdf')
    end
end
...
for i = 1:size(dayindex,1)
    allrecProps(i).cellTypeInfo = plotclassifiedwaveforms_K2(dayindex(i,:),...
        spikedatadir,savedatadir,allrecProps(i),params.thresholds,iden, plotdistperday);  
end
...
%% output
out.all.spikewidth = ALL_spikewidth;
out.all.meanFR = ALL_FR_mean;
out.all.ac = ALL_ac_mean;
out.pyr.spikewidth = ALL_spikewidth_PYR;
out.pyr.meanFR = ALL_FR_mean_PYR;
out.pyr.ac = ALL_ac_mean_PYR;
out.in.spikewidth = ALL_spikewidth_IN;
out.in.meanFR = ALL_FR_mean_IN;
out.in.ac = ALL_ac_mean_IN;

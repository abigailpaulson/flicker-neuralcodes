...
%% initialize figure
fh = figure('units','inch','position',[0,0,6.5,8]);
hold on

%% Panel A is a decoding example, I don't have that yet
ax_a1 = makesubplotwithletter(fh,4,4, 1, 'A', 'spanW', 1.3);
ax_a2 = makesubplotwithletter(fh,4,4, 3, 'A', 'spanW', 1.3);
...
ax_b1 = makesubplotwithletter(fh,4,4, 5, 'B');
ax_b2 = makesubplotwithletter(fh,4,4, 6, 'B');
ax_c = makesubplotwithletter(fh,4,4,7, 'C', 'spanW', 2);
ax_d1 = makesubplotwithletter(fh,4,4, 9, 'D');
ax_d2 = makesubplotwithletter(fh,4,4, 10, 'D');
ax_e = makesubplotwithletter(fh,4,4, 11, 'E', 'spanW', 2);
ax_f = makesubplotwithletter(fh,4,4, 13, 'F', 'spanW', 2);


cf_plot_prospectivecoding_byspeed(dirs, params, allindex, metadata, fh, ...
    {ax_b1, ax_b2, ax_c, ax_d1, ax_d2, ax_e, ax_f}, statsFID, {'C1', 'C2', 'E1', 'E2', 'F1', 'F2'}, tablefilename)

%% Determine the best LFP channels, mainly for CA3 region
...
% theta phase lag is not observed for CA3 theta, so only ripple power fit
% is used to determined the best CA3 channels
...
ThetaList=rmfield(ThetaList,'CA3ThChIndex');
ThetaList=rmfield(ThetaList,'CA3PyrChIndex');
...
for iCol=1:length(ChsortII)
   ...
   InputTemp=RippleList(i).CA3rippleRMSAve(ChsortII{iCol})/500;
   AMP=max(InputTemp)-min(InputTemp);
   x=ProbeXY(ChsortII{iCol},2);
   [r,r1]=max(InputTemp);

   %  fitFomula='(a*exp(-(x-b)^2/2/c))+d';
% %    figure;
% %    StartPoint=[AMP x(r1) std(x)^2 median(InputTemp)];
% %    LowerP=[AMP*0.1 min(x) std(x)^(-2) median(InputTemp)-std(InputTemp)];
% %    UpperP=[AMP*0.9 max(x) std(x)^2.5 median(InputTemp)+std(InputTemp)];
% % 

%    [RippleVecFit(:,iCol,i),ChRipple(i,iCol),xf,Rippleyf(:,i,iCol)]=ripplePowerVsDepth(ChsortII{iCol},RippleList(i).CA3rippleRMSAve(ChsortII{iCol})/500,ProbeXY(ChsortII{iCol},2),StartPoint,LowerP,UpperP);
   [RippleVecFit(:,iCol,i),ChRipple(i,iCol),xf,Rippleyf(:,i,iCol)]=ripplePowerVsDepth(ChsortII{iCol},RippleList(i).CA3rippleRMSAve(ChsortII{iCol})/500,ProbeXY(ChsortII{iCol},2));
 
   RippleAcc(i,iCol)=RippleVecFit(1,iCol,i)/std(RippleList(i).CA3rippleRMSAve(ChsortII{iCol})/500);
% %     RippleVecFit(2:5,iCol,i)
% %     
% %     plot(ProbeXY(ChsortII{iCol},2),RippleList(i).CA3rippleRMSAve(ChsortII{iCol})/500,'r*'); hold on 
% % % %     plot(ProbeXY(ChsortII{iCol},2),InputTemp,'b*');  
% %    
...
% %    set(gca,'xlim',[-300 1],'visible','off','clim',[0 0.4],'ylim',[0 0.006]);
   ...
   if isempty(ThetaList(i).CA1CCAll{iCol})
    ...
    plot(ProbeXY(ChsortII{iCol},2),RippleList(i).CA3rippleRMSAve(ChsortII{iCol})/500,'r.');   
    hold on;
    if RippleAcc(i,iCol)<=rTh
    plot(xf,Rippleyf(:,i,iCol),'r-','linewidth',1);
     text(xf(20),Rippleyf(20),['rmse' showNum(RippleAcc(i,iCol),4)],...
...
set(gcf, 'PaperUnits', 'centimeters');
set(gcf,'PaperPosition',papersizePX,'PaperSize',papersizePX(3:4));
saveas(gcf,[SaveData 'ThetaRippleCA3ShankColFitAcc'],'tiff'); 
% saveas(gcf,[SaveData 'ThetaRippleCA3ShankFitAcc'],'tiff'); 
...
Acc=RippleAcc;    %%%Only ripple power fit is considered, not the theta phase lag
rTh=0.5;
tTh=0.5;
...
% RTdistVI=RTdist<0&RTdist>-150;
% RTdistVI2=RTdist<0;

RVI=RippleAcc<=rTh;   %%%Only ripple power fit is considered, not the theta phase lag
% TVI=ThetaAcc<=tTh;

ValidVI=(RVI+ValidI)==2;
...
ThetaList(i).CA3ThChIndex=[];
ThetaList(i).CA3PyrChIndex=[];
...
% %    end
    ...
    plot(xf,Rippleyf(:,i,iCol),'r-','linewidth',1);
     text(xf(20),Rippleyf(20),['rmse' showNum(RippleAcc(i,iCol),4)],...
       'fontsize',8,'fontname','times new roman','horizontalalignment','center','color','r')
    plot(ProbeXY(ChRipple(i,iCol),2),RippleList(i).CA3rippleRMSAve(ChRipple(i,iCol))/500,'r*');
   
    ThetaList(i).CA3PyrChIndex=[ThetaList(i).CA3PyrChIndex ChRipple(i,iCol)];    
...
set(gcf, 'PaperUnits', 'centimeters');
set(gcf,'PaperPosition',papersizePX,'PaperSize',papersizePX(3:4));
saveas(gcf,[SaveData 'CA3RippleShankColFitAccOpt'],'tiff'); 


save([SaveData 'CA3ShankColAlignPyrOriCh.mat'],'ThetaList','AccSh','RippleAcc','RippleVecFit','Acc','Rippleyf',...
    'ProbeXY','ChsortII','ChsortSh','ChRipple');
